...
/*************************************************************/
/*  [ setting note ]                                         */
/*  Si diamond (Si monoatomic system)                        */
/*  one unit cell with 8 atoms                               */
/*                                 1999. Dec. project IEMD   */
/*************************************************************/

void  set_param(void)
{
  wv.E_cut  = 3.0;   /* plane wave cut off energy [Hartree]               */
  wv.E_cuti = 1.0;   /* cutoff energy at initial stage of diagonalization */
                     /* E_cut > E_cuti                                    */

  /*--- approach to the Born-Oppenheimer potential surface ---------------*/
  cpq.dt = 4.0;            /* = delta t [a.u.]                */
  cpq.myu = 200.0;	   /* pseudo mass of electron [a.u.]  */
  cpq.residual = 1.0e-6;   /* allowed residual                */
...
void	set_atoms(void)
{
  ...
  atom.ntypes = 1;  /* total types of atoms = 1 (monoatomic system) */
  ...
  for(i=0;i<atom.N;i++){
    atom.type[i] = type;                      /* all atoms are Si           */
  }
  /*------------------------------------------------------------------------*/
