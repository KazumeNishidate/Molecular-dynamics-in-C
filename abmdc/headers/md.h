...
typedef struct{
  double E[FFTPTSX*FFTPTSY*FFTPTSZ];  /* Exc[rho]                           */
  double V[FFTPTSX*FFTPTSY*FFTPTSZ];  /* Vxc = dExc/drho                    */
  double r[FFTPTSX*FFTPTSY*FFTPTSZ];  /* VH(G) + Vxc(G) (real part)         */
  double i[FFTPTSX*FFTPTSY*FFTPTSZ];  /* VH(G) + Vxc(G) (imaginary part)    */
  double VH[FFTPTSX*FFTPTSY*FFTPTSZ]; /* calclate the Hartree in real space */
} exchange_correlation_set;

typedef struct{
  double ekin;    /* electronic kinetic energy                           */
  double fkin;    /* fictitious kinetic energy [Real]                    */
  double ckin;    /* core (ionic) kinetic energy                         */
  double ckinx;     /* X component of the core (ionic) kinetic energy      */
  double ckiny;     /* X component of the core (ionic) kinetic energy      */
  double ckinz;     /* X component of the core (ionic) kinetic energy      */
  double VpsR;    /* Vps energy [Real]                                   */
  double hart;    /* Hartree energy (bug)                                */
  double xc;      /* exchange + correlation energy                       */
  double EW;      /* EWALD potential                                     */
  double eTotal;  /* E.ekin + E.VpsR + E.hart + E.xc                     */
  double cTotal;  /* E.ckin + E.EW                                       */
  double TotalF;  /* E.eTotal + E.cTotal + E.fker                        */
  double Total;   /* E.eTotal + E.cTotal                                 */
  double eTemp;   /* electronic temperature [K]                          */
  double efTemp;  /* fictitious electronic temperature [K]               */
  double cTemp;   /* ionic temperature [K]                               */
} total_energy_set;
...
typedef struct{
  double value[MAX_ATOM_TYPE];
  int number_of_ion[MAX_ATOM_TYPE];
} msd_calculation;
