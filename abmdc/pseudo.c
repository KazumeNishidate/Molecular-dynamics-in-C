  ...
  for(nq1=0;nq1<n_pl;nq1++) {
    ...
    for(nq2=0;nq2<n_pl;nq2++) {
      ...
      for(i=0;i<atom.N;i++) {
        ...
        calc_core_Vps(atom.type[i], nq1, nq2, kx, ky, kz);
        
        if(nq1==0 || nq2 ==0) { /* angular momentum dependent part */
          calc_delta_Vps_zero(atom.type[i], nq1, nq2, kx, ky, kz);
        } else {
          calc_delta_Vps(atom.type[i], nq1, nq2, kx, ky, kz);
        }
...
void   set_Vps_hmat(int n_pl, double *kx, double *ky, double *kz)
{
  ...
  for(nq1=0;nq1<n_pl;nq1++) {
	...
	/*------------------------------------------------------*/
	ggr = dgx*cell.rx[i]+dgy*cell.ry[i]+dgz*cell.rz[i];
	hm.mat_re[cnt] += cos(ggr)*wk.Vps;
	hm.mat_im[cnt] += sin(ggr)*wk.Vps;
      }
      cnt++;
    }
  }
		   ...
		   double *kx, double *ky, double *kz)
{
  ...
  if(nq1==nq2) {
    wk.Vps = PI*atom.Zv[type]*( c1/a1 + c2/a2 );
  } else {
