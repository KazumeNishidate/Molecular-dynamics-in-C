...
void	real_space(void) {
  ...
  real_vdW();  /* include the vdW force */
}

void	real_vdW(void) {  /* vdW force for graphite */
  int i, j;
  double rdx, rdy, rdz;
  double rij, xij, yij, zij;
  double dr8, dr14, vdW_F;
  ...
  for(i=0;i<sys.N;i++) {
    for(j=i+1;j<sys.N;j++) {
      ...
      /* chechk the working region for vdW force */
      if(rij < vdW.r1 || rij > vdW.r2) continue ;

      /*  differential form of the two-body LJ potential     */
      /*    "SIMULATION BUTSURIGAKU", by D. W. Heermann      */
      /*	       Japanese edt., Springer-Verlag (1990) */

      dr8  = rij*rij*rij*rij*rij*rij*rij*rij;
      dr14 = dr8*rij*rij*rij*rij*rij*rij;
      vdW_F = vdW.eps_sig2*(vdW.sig14/dr14 - 0.5*(vdW.sig8/dr8) );

      sys.fx[i] += vdW_F*xij; /* action */
      sys.fy[i] += vdW_F*yij;
      sys.fz[i] += vdW_F*zij;
 
      sys.fx[j] -= vdW_F*xij; /* reaction */
      sys.fy[j] -= vdW_F*yij;
      sys.fz[j] -= vdW_F*zij;
