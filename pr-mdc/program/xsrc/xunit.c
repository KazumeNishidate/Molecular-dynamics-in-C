...
void   open_xunit(void)   /* XUNIT */
{
  ...
  x_scale = 200.0/cell.hm[0][0];
  y_scale = 200.0/cell.hm[1][1];
  z_scale = 200.0/cell.hm[2][2];

  unit_x = 1.1*cell.hm[0][0]/(double)cell.nx;
  unit_y = 1.1*cell.hm[1][1]/(double)cell.ny;
  unit_z = 1.1*cell.hm[2][2]/(double)cell.nz;
  ...
  for(i=0; i<sys.N; i++) {
    if(cell.rx[i]<unit_x && cell.ry[i]<unit_y && cell.rz[i]<unit_z){

      ptx = (int)(cell.rx[i]*x_scale)+9;
      pty = (int)(cell.ry[i]*y_scale)+9;
      ptz = (int)(cell.rz[i]*z_scale)+9;
