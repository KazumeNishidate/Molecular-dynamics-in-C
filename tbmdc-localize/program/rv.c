...
void    calc_foc(void)
{
  ...
  local_lookup();  /* list up the ROCAL atoms */
  repulsive();     /* repulsive force and potential */

  set_chemical_potential(); /* taka : Tue May  2 17:51:48 JST 2000 */

  for(target_atom=0;target_atom<sys.N;target_atom++) {
    /*    printf("target_atom = %d\n",target_atom); */
    hamiltonian(target_atom);

    /* --- taka (Wednesday, June 7, 2000) --- */
    /* As the TB energy is already calculated in 'set_chemical_potential()',
       'calc_Fermi_E(target_atom)' doesn't have to call here. */

    /* calc_Fermi_E(target_atom);   calculate the bond structure energy */

    sys.pot += htb.potential[target_atom];  /* sys.pot = Erep + Eel     */
    calc_Fermi_F(target_atom);
  }

}

/*** 
  Verlet's velocity form: L. Verlet, Phys. Rev. 136. A405 (1964)
  see also "Computer Simulation Methods in Theoretical Physics"
...
void next_rv_gear(void)  /* Gear's 7th-order F representation */
{
  static int cnt=0;
