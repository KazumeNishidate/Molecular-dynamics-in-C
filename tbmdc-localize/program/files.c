...
void   print_to_file(void)
{
  ...
  fprintf(fpout,"%d  %.2f %.2f %.2f  %.2f %.2f  %.3f %.3f %.3f  %.5f",
         sys.step,
         ...
         sys.kin*sys.e2t,
         sys.pres*sys.pp2gpa,
         sys.Lx/sys.nx,sys.Ly/sys.ny, sys.Lz/sys.nz, ch.myu/htb.eV2E);
...
void   display4(void)
{
  ...
  if(sys.step % 10 == 1){
    printf("  -----------------------------------------------------------------\
-------\n");
    printf("  step: Ek : Up  : Et  : T  :  P   :  ( x  y  z )   :  myu  :  MSD\n");
    printf("  -----------------------------------------------------------------\
-------\n");
  };

  printf("%6u %4.2f %4.2f %4.2f %4.2f %5.2f  (%3.2f %3.2f %3.2f)  %3.2f    ",
         sys.step,
         sys.kin/(double)sys.N/htb.eV2E,
         sys.pot/(double)sys.N/htb.eV2E,
         (sys.kin+sys.pot)/(double)sys.N/htb.eV2E,
