...
void get_control_param(void)
{
  ...
  ctl.calc_max = 100;  /* maximum MD time step                     */
  delta_time_fs = 0.7;   /* 1 [fs] = 0.001 [ps] = 1.0 X 10^(-15) [s] */

  ctl.temp = 0.0;       /* Temperature setting [K] */
  ctl.t_control_step = 5;
  set_press_GPa_X = 0.0;  /* Pressure setting  [GPa] */
  set_press_GPa_Y = 0.0; 
  set_press_GPa_Z = 0.0; 
  ...
  /* check */
  sys.Ax = 3.55984;   /* initial lattice constant */
  sys.Ay = 3.55984;     /* initial lattice constant */
  sys.Az = 3.55984;     /* initial lattice constant */
  ...
  /* check */
  sys.N = 8*sys.nx*sys.ny*sys.nz;  /* total number of ions [diamond strct.] */

  /* ----- taka (Thursday, June 8, 2000) ----- */
  multiwall_nanotube( 1, 5, 5, 0, 0 );
  sys.Ax = 50.0;
  sys.Ay = multi.T;
  sys.Az = 50.0;

  sys.Ax = sys.Az = 2*multi.rad+3.2;

  sys.N = multi.atoms*sys.nx*sys.ny*sys.nz;
  /* ------------------------------------------ */
...
void	set_roc(void)
{
  ...
  printf( "--- carbon nanotube --- \n" );
  for( i = 0; i < sys.N; i++ ){
    sys.rx[ i ] = multi.rx[ i ]+shiftx;
    sys.ry[ i ] = multi.ry[ i ];
    sys.rz[ i ] = multi.rz[ i ]+shiftz;
  }
  printf( "the number of atoms : %d x %d (%d)\n",
           multi.atoms, sys.ny, sys.N );

  delete_multi_nanotube();
}
